...
func main() {
	...
	//3DES 24
	key := []byte("hahahahaxixixixidadadada")
	result,err := Des3Encrypt([]byte("mima"),key) //加密
	if err != nil {
		panic(err)
	}
	fmt.Println("加密后数据",base64.StdEncoding.EncodeToString(result))
	origData,err := Des3Decrypt(result,key) //解密
	if err != nil {
		panic(err)
	}
...
func Des3Encrypt(origData, key []byte) ([]byte, error) {
	block,err := des.NewTripleDESCipher(key)		//key作为加密密钥
	if err != nil {
		return nil,err
	}
